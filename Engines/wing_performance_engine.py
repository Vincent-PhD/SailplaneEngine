...
def get_wing_performance(ga_instance, solution, solution_idx):
    ...
    _LengthPenalty = np.abs((15 - (2 * np.sum(increments))))
    ...
    return obj_func
    # except:
    #     print("Exception Raised Due to Module Convergence Failure")
    #     return abs(
    #         planform_penalty(x=chords, weight=340_000)
    #         + np.abs((15 - (2 * np.sum(increments)))) * 1e3
    #         + _Total_Airfoil_Penalty
    #     )
